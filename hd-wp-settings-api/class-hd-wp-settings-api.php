...
if ( ! class_exists( 'HELPC_WP_Settings_API' ) ) :
/**
 * WordPress Settings API Wrapper Class
 *
 * @version 1.1
...
class HELPC_WP_Settings_API {
	...
	/**
	 * Holds instance of HELPC_HTML_Helper class
	 * @var object
	 */
	var $html_helper;
	...
	function __construct( $options = array(), $fields = array() ) {
		...
		$this->html_helper = class_exists( 'HELPC_HTML_Helper' ) ? new HELPC_HTML_Helper : false;
	...
	function register_menu() {
		...
		if ( ! empty( $this->tabs ) ) {
			...
			elseif ( isset( $_REQUEST[ $this->options['menu_slug'] . '_active_tab' ] ) && array_key_exists( $_REQUEST[ $this->options['menu_slug'] . '_active_tab' ], (array) $this->tabs ) )
				$this->active_tab = sanitize_text_field($_REQUEST[ $this->options['menu_slug'] . '_active_tab' ]);
			else {
